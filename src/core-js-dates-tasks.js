...
function dateToTimestamp(date) {
  const comparebleDate = new Date(date);
  const beginningDate = new Date(0);

  return comparebleDate - beginningDate;
}
...
function getCountDaysInMonth(month, year) {
  const beginning = new Date(year, month - 1, 0);
  const end = new Date(year, month, 0);

  return Math.abs(end - beginning) / (1000 * 60 * 60 * 24);
}
